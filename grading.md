...
INST1=$(aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=efs-ec2-1" "Name=instance-state-name,Values=running" \
  --query 'Reservations[0].Instances[0].InstanceId' --output text 2>/dev/null)
if [ "$INST1" != "None" ] && [ -n "$INST1" ]; then
  CMD1=$(aws ssm send-command --instance-ids $INST1 --document-name "AWS-RunShellScript" \
    --parameters 'commands=["df -h /mnt/efs | grep -E nfs"]' \
  ...
  check "FAIL" "EC2 #1 efs-ec2-1 없음"
fi

INST2=$(aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=efs-ec2-2" "Name=instance-state-name,Values=running" \
  --query 'Reservations[0].Instances[0].InstanceId' --output text 2>/dev/null)
if [ "$INST2" != "None" ] && [ -n "$INST2" ]; then
  ...
  SHARE=$(aws ssm get-command-invocation --command-id $CMD2 --instance-id $INST2 \
    --query 'StandardOutputContent' --output text 2>/dev/null)
  echo "$SHARE" | grep -q "hello-efs" && check "PASS" "EC2 #2 공유 파일 읽기" || check "FAIL" "EC2 #2 공유 파일 읽기"
else
  check "FAIL" "EC2 #2 efs-ec2-2 없음"
fi
...
BUCKET="athena-lab-${ACCT}"
aws s3 ls s3://$BUCKET/ >/dev/null 2>&1 && check "PASS" "S3 버킷 존재" || check "FAIL" "S3 버킷 존재"

aws s3 ls s3://$BUCKET/data/employees.csv >/dev/null 2>&1 && check "PASS" "CSV 파일 존재" || check "FAIL" "CSV 파일 존재"

aws athena get-work-group --work-group my-workgroup >/dev/null 2>&1 && check "PASS" "Athena WorkGroup 존재" || check "FAIL" "Athena WorkGroup 존재"

QUERY_ID=$(aws athena start-query-execution \
  --query-string "SELECT count(*) as cnt FROM mydb.employees;" \
...
if [ -n "$QUERY_ID" ]; then
  ...
  [ "$RESULT" = "5" ] && check "PASS" "Athena 쿼리 결과 5건 확인" || check "FAIL" "Athena 쿼리 결과 ${RESULT}건"
else
...
aws iam get-user --user-name lab-user >/dev/null 2>&1 && check "PASS" "IAM 사용자 lab-user 존재" || check "FAIL" "IAM 사용자 lab-user 존재"

ATTACHED=$(aws iam list-attached-user-policies --user-name lab-user \
  --query 'length(AttachedPolicies)' --output text 2>/dev/null)
[ "$ATTACHED" -ge 1 ] 2>/dev/null && check "PASS" "정책 연결됨" || check "FAIL" "정책 연결됨"

USER_ARN=$(aws iam get-user --user-name lab-user --query 'User.Arn' --output text 2>/dev/null)
GET_RESULT=$(aws iam simulate-principal-policy \
  --policy-source-arn $USER_ARN \
  --action-names s3:GetObject \
  --resource-arns "arn:aws:s3:::${BUCKET}/*" \
  --query 'EvaluationResults[0].EvalDecision' --output text 2>/dev/null)
[ "$GET_RESULT" = "allowed" ] && check "PASS" "s3:GetObject = allowed" || check "FAIL" "s3:GetObject = $GET_RESULT"

PUT_RESULT=$(aws iam simulate-principal-policy \
  --policy-source-arn $USER_ARN \
  --action-names s3:PutObject \
  --resource-arns "arn:aws:s3:::${BUCKET}/*" \
  --query 'EvaluationResults[0].EvalDecision' --output text 2>/dev/null)
[ "$PUT_RESULT" = "implicitDeny" ] && check "PASS" "s3:PutObject = implicitDeny" || check "FAIL" "s3:PutObject = $PUT_RESULT"

POLICY_ARN=$(aws iam list-attached-user-policies --user-name lab-user \
...
RESOURCE=$(aws iam get-policy-version --policy-arn $POLICY_ARN --version-id $POLICY_VER \
  --query 'PolicyVersion.Document.Statement[0].Resource' --output text 2>/dev/null)
echo "$RESOURCE" | grep -q "arn:aws:s3" && check "PASS" "Resource 특정 버킷 지정" || check "FAIL" "Resource가 * - Fine-grained 아님"

echo ""
echo "[문제 4] RDS + Lambda - MySQL with Lambda"
echo "------------------------------------------"

RDS_STATUS=$(aws rds describe-db-instances --db-instance-identifier my-lab-db \
  --query 'DBInstances[0].DBInstanceStatus' --output text 2>/dev/null)
[ "$RDS_STATUS" = "available" ] && check "PASS" "RDS MySQL available" || check "FAIL" "RDS MySQL - $RDS_STATUS"

RDS_ENGINE=$(aws rds describe-db-instances --db-instance-identifier my-lab-db \
  --query 'DBInstances[0].Engine' --output text 2>/dev/null)
[ "$RDS_ENGINE" = "mysql" ] && check "PASS" "RDS 엔진 = mysql" || check "FAIL" "RDS 엔진 = $RDS_ENGINE"

aws lambda get-function --function-name rds-connector >/dev/null 2>&1 && check "PASS" "Lambda 함수 존재" || check "FAIL" "Lambda 함수 존재"

LAMBDA_VPC=$(aws lambda get-function-configuration --function-name rds-connector \
...
RDS_EP=$(aws rds describe-db-instances --db-instance-identifier my-lab-db \
  --query 'DBInstances[0].Endpoint.Address' --output text 2>/dev/null)
aws lambda invoke \
  --function-name rds-connector \
  --payload '{"host":"'"$RDS_EP"'","port":3306}' \
  --cli-binary-format raw-in-base64-out \
  /tmp/grade_output.json >/dev/null 2>&1
LAMBDA_RESULT=$(cat /tmp/grade_output.json 2>/dev/null)
echo "$LAMBDA_RESULT" | grep -q "successful" && check "PASS" "Lambda -> RDS 연결 성공" || check "FAIL" "Lambda -> RDS 연결 실패"
